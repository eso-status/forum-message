...
export default class ForumMessageElement {
  ...
  public static getDate(rawDate: string): Moment[] {
    ...
    if (rawDate.includes(' at ') && !rawDate.includes('EDT / ')) {
      const dateTime: RegExpExecArray | null = /([a-zA-Z]{6,9}) at [0-9]{1,2}:[0-9]{1,2}[a-zA-Z]{1,2} EDT \(([0-9]{1,2}):([0-9]{1,2}) UTC\)/.exec(rawDate);
      const isoWeekDay: number = Number(moment().day(dateTime && dateTime.length >= 1 ? String(dateTime[1]) : '').format('d'));
